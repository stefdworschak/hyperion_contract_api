...
contract Validator {
    ...
    function validateOne(string memory email, bytes32 documentHash)
            public view returns(bytes32[] memory validated) {
        bytes32[] memory validatedDocuments = new bytes32[](1);
        bytes32 emailHash = hashString(email);
        if(validate(emailHash, documentHash)) {
            validatedDocuments[0] = documentHash;
        }
